...
func TestStartTimeMetricMatch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stma := NewStartTimeMetricAdjuster(zap.NewNop(), tt.startTimeMetricRegex, false, gcInterval)
			...
			assert.NoError(t, stma.AdjustMetrics(pmetrics))
...
func TestStartTimeMetricFallback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gcInterval := 10 * time.Millisecond
			stma := NewStartTimeMetricAdjuster(zap.NewNop(), tt.startTimeMetricRegex, true, gcInterval)
			if tt.expectedErr != nil {
				assert.ErrorIs(t, stma.AdjustMetrics(tt.inputs), tt.expectedErr)
				return
			}
...
func TestFallbackAndReset(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gcInterval := 10 * time.Millisecond
			stma := NewStartTimeMetricAdjuster(zap.NewNop(), nil, true, gcInterval)
